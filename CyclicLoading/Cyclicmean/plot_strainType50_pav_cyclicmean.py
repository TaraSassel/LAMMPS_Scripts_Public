...
Recreating Wichtmann et al. 2007 Figure 8 with my data
...
for p, path in enumerate(path_list):
    ...
    p_val = np.arange(0,400)

    # eps_val = [value**(0.3)/20 for value in p_val]
    # plt.plot(p_val,eps_val, lw = 0.5, ls = '--', c = 'black')

    eps_val = [value**(0.5)/68 for value in p_val]
    plt.plot(p_val,eps_val, lw = 0.5, ls = '--', c = 'black')
    eps_val = [value**(0.5)/21 for value in p_val]
    plt.plot(p_val,eps_val, lw = 0.5, ls = '--', c = 'black')
    eps_val = [value**(0.5)/23 for value in p_val]
    plt.plot(p_val,eps_val, lw = 0.5, ls = '--', c = 'black')
...
plt.xlim(0,350)
plt.ylim(0,)
plt.xlabel('Average mean pressure ($p^{av}) [kPa]$', fontsize = LBF)
plt.ylabel('Strain amplitude ($\epsilon^{ampl}$) [$10^{-4}$]', fontsize = LBF)
plt.grid(ls = '--', lw = 1, color = 'gray')
plt.gca().tick_params(which = 'major', labelsize = TS)
plt.tight_layout()

os.chdir(r'C:\Users\taras\My Drive\PhD\My Writing\Paper_CyclicMean')
plt.savefig(f'StrainAmplitude_Pav_{which_data}_cyclicmean.png', format='png')
plt.savefig(f'StrainAmplitude_Pav_{which_data}_cyclicmean.eps', format='eps')
plt.show()
