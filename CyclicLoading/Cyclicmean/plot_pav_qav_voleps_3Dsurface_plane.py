...
# ==============================================================================
path_list, color_list =  get_cyclicmean_data('all_V2','E')
...
for p, path in enumerate(path_list):
    ...
    strain_values.append(list(eps_v_amp)[0])
...
# 0.08184898954906916 zmax value for figure (a)

# colorbar
m = cm.ScalarMappable(cmap=cm.viridis)
m.set_array(Z)
cbar = plt.colorbar(m, fraction = 0.03, pad = 0.1, alpha = 0.5)
cbar.ax.set_ylabel('Strain amplitude $(\epsilon_{v}^{ampl})$ [%]', fontsize = LBF)
...
ax.set_zlabel('Volumetric strain amplitude $(\epsilon_{v}^{ampl})$ [%]', fontsize = LBF, labelpad=10, rotation = 90)
