...
LGF = 9    # Legend Font Size
LBF = 14    # Label Font Size
TS = 14     # Tick Size
FS1 = 9    # font size for text 
me1 = 500   # Marker spacing
...
path0 = r'F:\PhD_Data_Back_Up\LAMMPS\Random_Packing\Istotropic Compression\Toyoura15k\NoFix_ISOCOMP200_Density2650_FC0p25'
os.chdir(path0)
# Names of Files
...
line2 = plt.plot(timestep[0:a], W1[0:a],'grey',marker = 'x',markevery=me1,markersize = mz1,label=r'$W_{bound\:simulated}$',linewidth = 5)
line2 = plt.plot(timestep[0:a], W2[0:a],'darkred',markevery=me1,markersize = mz1,label=r'$W_{vol\:simulated} + W_{dist\:simulated}$',linewidth = 2)
line3 = plt.plot(timestep[1:a], W3[0:a],'darkgreen',markevery=me1,markersize = mz1,label=r'$W_{vol\:simulated} + W_{dist\:modified}$')
...
# Figure Speedup
ax2=ax.twinx()
ax2.plot(timestep[0:a],q[0:a]/1000,color = 'lightcoral', label = r'Simulated deviatoric stress $q = \sqrt{\frac{1}{2}[(\sigma_{xx}-\sigma_{yy})^2+(\sigma_{xx}-\sigma_{zz})^2+(\sigma_{yy}-\sigma_{zz})^2+3\times(\sigma_{xy}^2+\sigma_{xz}^2+\sigma_{yz}^2)]}$',linewidth = 2)
ax2.plot(timestep[0:a],q_new[0:a]/1000,color = 'yellowgreen', label = r'Modified deviatoric stress $q = \sigma_{zz}-\frac{1}{2}(\sigma_{xx}-\sigma_{yy})$',linewidth = 2)
# Labels
ax2.set_ylabel('Deviatoric stress [kPa]',fontsize = LBF)
ax2.set_yticks(np.linspace(-60,60,13))
max_W3error = 0.00004
print(max_W3error)
ax2.set_ylim(-60,60)
ax.set_ylim(-max_W3error,max_W3error)
ax2.legend(fontsize = LGF,loc = 'lower right')
leg = ax.legend(fontsize = LGF,loc = 'lower left',facecolor = 'white',framealpha = 1)
...
print(cycle_range)
#Arrow 1
plt.arrow(0.8e8, 40,cycle_range, 0, length_includes_head=True ,head_width=2, head_length=500000,color='k')
plt.arrow(0.8e8+cycle_range, 40,-cycle_range, 0, length_includes_head=True ,head_width=2, head_length=500000,color='k')
plt.text(0.8e8+cycle_range/2,42, r'Positive distortional' +  '\n' +'energy', fontsize = FS1,ha='center')
#Arrow 1
plt.arrow(0.8e8+cycle_range, 40,cycle_range, 0, length_includes_head=True ,head_width=2, head_length=500000,color='k')
plt.arrow(0.8e8+cycle_range*2, 40,-cycle_range, 0, length_includes_head=True ,head_width=2, head_length=500000,color='k')
plt.text(0.8e8+cycle_range*(3/2),42, r'Negative distortional' +  '\n' +'energy', fontsize = FS1,ha='center')
#Vertical Dotted lines
ax2.plot([0.8e8+cycle_range,0.8e8+cycle_range],[-100,100],linestyle='dotted',color = 'k')
ax2.plot([0.8e8+cycle_range*2,0.8e8+cycle_range*2],[-100,100],linestyle='dotted',color = 'k')
plt.show()
# ###### save plots--------------------------------------------------------------
# os.chdir(r'C:\Users\shrab\Google Drive\PhD\Meetings\200610_Energy')
# f.savefig('EnergyBalanceError_TXShear300kPa_fromFC0p2to0p25',  bbox_inches='tight',pad_inches=0)
# i.savefig('samplefigure2',  bbox_inches='tight')
# j.savefig('samplefigure3',  bbox_inches='tight')
###### Show all plots----------------------------------------------------------

# # plt.tight_layout()
# plt.savefig("check_W_cyclic_mod.png")
#
# # Show  ---------------------------------------------------------------------
# #plt.show()
# Image.open("check_W_cyclic_mod.png").show()
