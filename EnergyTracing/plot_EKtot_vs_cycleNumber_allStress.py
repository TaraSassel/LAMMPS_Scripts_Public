...
plt.ylabel('Total kinetic energy ($E_{k} = E_{kr} + E_{kt}$) [J]', fontsize = LBF)
...
lg1 = ax.legend(handles=[line1, line2, line3], fontsize = LGF, title = "$\epsilon_{zz}^{ampl}$:", loc='lower left', bbox_to_anchor=(1, 0.01))

# lg2:
patch1 = mlines.Line2D([], [], color = color_list[0], lw = lw1, label = "$p'_0$ = 100 kPa" + "\n" + r"$\mu = 0.25$")
patch2 = mlines.Line2D([], [], color = color_list[1], lw = lw1, label = "$p'_0$ = 200 kPa" + "\n" + r"$\mu = 0.25$")
patch3 = mlines.Line2D([], [], color = color_list[2], lw = lw1, label = "$p'_0$ = 300 kPa" + "\n" + r"$\mu = 0.25$")
patch4 = mlines.Line2D([], [], color = 'black', lw = lw1, marker ='s', label = "$p'_0$ = 300 kPa" + "\n" + r"$\mu = 0.15$")

lg2 = ax.legend(handles=[patch1, patch2, patch3, patch4], fontsize = LGF, loc='upper left', bbox_to_anchor=(1, 1))
